...
import type { Order, Profile, ShippingAddress } from '@/lib/types'
...
export default function ProfilePage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (p) {
                ...
                setEditAddress(p.address ?? EMPTY_ADDRESS)
            }
            setLoading(false)
        }
        fetchData()
    ...
    const displayName = profile.full_name || authEmail?.split('@')[0] || 'Customer'
    const initials = getInitials(profile.full_name, authEmail)
            ...
            <div className="px-4 py-4 space-y-3 max-w-2xl mx-auto">
                ...
                <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-[#F5F5F5]">
                        ...
                        {!editing ? (
                            <button onClick={startEdit} className="flex items-center gap-1.5 text-xs font-semibold text-[#FF6600] hover:opacity-75">
                                <Pencil size={13} /> Edit
                            </button>
                        ) : (
                            <div className="flex items-center gap-3">
                                <button onClick={cancelEdit} className="flex items-center gap-1 text-xs font-semibold text-[#767676] hover:opacity-75">
                                    <X size={13} /> Cancel
                                </button>
                                <button
                                    onClick={saveProfile}
                                    disabled={saving}
                                    className="flex items-center gap-1 text-xs font-semibold text-green-600 hover:opacity-75 disabled:opacity-50"
                                >
                    ...
                    <div className="px-4 py-3 space-y-4">
                        ...
                        {/* Full Name */}
                        <div>
                            <label className="text-xs text-[#767676] font-semibold flex items-center gap-1.5 mb-1">
                            ...
                            {editing ? (
                                ...
                                <p className="text-sm text-[#2E2E2E]">{profile.full_name || <span className="text-[#B0B0B0]">Not set</span>}</p>
                            )}
                        </div>
                        ...
                        <div>
                            ...
                            {editing ? (
                                ...
                                <p className="text-sm text-[#2E2E2E]">{profile.phone || <span className="text-[#B0B0B0]">Not set</span>}</p>
                            )}
                        </div>
                            ...
                            {editing ? (
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        onChange={e => setEditAddress(a => ({ ...a, pincode: e.target.value }))}
                                        placeholder="Pincode"
                                        maxLength={6}
                                        type="tel"
                                        className="w-full bg-[#F5F5F5] border border-[#E8E8E8] rounded-lg px-3 py-2 text-sm outline-none focus:border-[#FF6600]"
                ...
                <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-[#F5F5F5]">
                        <p className="text-sm font-bold text-[#2E2E2E] flex items-center gap-2">
                            <Clock size={15} className="text-[#FF6600]" /> Recent Orders
                        </p>
                    ...
                    {recentOrders.length === 0 ? (
                        <div className="py-8 flex flex-col items-center text-center">
                            ...
                            <Link href="/categories" className="text-xs font-semibold text-[#FF6600] mt-2 hover:opacity-75">
                                Start Shopping →
                            </Link>
                        </div>
                    ) : (
                        <div className="divide-y divide-[#F5F5F5]">
                            {recentOrders.map(order => (
                                ...
                                >
                                    <div className="w-12 h-12 rounded-lg overflow-hidden bg-[#F9F0EC] shrink-0">
                                        {order.items[0]?.image ? (
                                            <img src={order.items[0].image} alt="" className="w-full h-full object-cover" />
                                        ) : (
                                            <div className="w-full h-full flex items-center justify-center">
                                                <Flower2 size={20} className="text-[#FF6600] opacity-30" />
                                            </div>
                                        )}
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <p className="text-xs text-[#767676]">
                                            #{order.id.slice(0, 8).toUpperCase()} · {new Date(order.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
                                        </p>
                                        <p className="text-sm font-semibold text-[#2E2E2E] truncate">
                                            {order.items.map(i => i.name).join(', ')}
                                        </p>
                                        <span className={`badge text-[10px] mt-0.5 inline-block ${STATUS_COLORS[order.status] || 'bg-gray-100 text-gray-700'}`}>
